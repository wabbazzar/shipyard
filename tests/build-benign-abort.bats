...
@test "build: installer-owned .agents/shoulder.env is not dirty" {
  P="$(make_fixture_project blshoulder can-merge-false.toml)"
  mkdir -p "$P/.agents"
  printf '# generated by install.sh --wire-shoulder\nCRITIC_PROJECT_DIR=%s\n' "$P" \
    > "$P/.agents/shoulder.env"

  # Stop deterministically immediately after pre-flight, without a model call.
  printf '%s\n' '{"svc":"blshoulder-build","event":"job.end","tokens":1000000}' \
    >> "$(events_file)"
  run run_runner build "$P" --mode live
