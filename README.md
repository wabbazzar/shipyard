...
| synchronous hook cap | `[release] hook_wall_clock_sec` — `--mode hook` blocks whoever invoked it, so it runs against a **deadline for the whole run**: the model spawn, `[release.blocking_gate]`, and `verify_gate` re-runs all draw from the same remaining seconds instead of each spending its own on top. Out of budget → the stage is skipped and the verdict is `incomplete` (**DID NOT FINISH**, no medic escalation, no dispatch item), never a false failure. An explicit value wins; otherwise a tighter `wall_clock_sec` is never loosened by the default. `--mode daily` keeps the full `wall_clock_sec`. | **600s** (`wall_clock_sec` if lower) |
