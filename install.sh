...
write_skill_bridge() {
  ...
  fi
  {
    ...
    printf 'Codex and Hermes auto-inject this file when opened in the project.\n'
    printf 'The reusable skills for this project live under\n'
...
# run_relink — repair installer-owned skill symlinks that --doctor (check e)
# flags as drift and create a missing skill bridge (check f). These deterministic,
# reversible filesystem ops touch no systemd, config, or gate file and never
# clobber a real file/dir an operator placed there. Honors --dry-run. Exit 0
# even when nothing needed fixing; exit 2 only when the source skills dir is gone.
run_relink() {
  local qskills
