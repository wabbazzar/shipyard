...
run_doctor() {
  ...
  # (i) shoulder-mode wiring drift — only when this project OPTED IN (config
...
# claude auto-discovers .claude/skills; codex and hermes do not, but both
# auto-inject an AGENTS.md from the working directory (the unit runs with
# WorkingDirectory=$PROJECT_DIR, so a root AGENTS.md is in scope). If any
# installed role runs on a non-claude harness, drop a bridge AGENTS.md that
# points those harnesses at the symlinked skills. claude reads CLAUDE.md, not
# AGENTS.md, so this file is inert for claude-only roles. NEVER clobber an
# existing AGENTS.md (the operator or another tool may own it).
foreign_harness=0
for role in $ROLES_LIST; do
  rh="$(jq -r --arg r "$role" '(.[$r].harness // .harness.default // empty)' <<<"$CFG_JSON")"
  case "$rh" in codex|hermes) foreign_harness=1 ;; esac
done
AGENTS_DEST="$PROJECT_DIR/AGENTS.md"
if [ "$foreign_harness" = "1" ]; then
  if [ -f "$AGENTS_DEST" ]; then
    echo "  AGENTS.md: exists — leaving as-is (never clobbered)"
  elif [ "$DRY_RUN" = "1" ]; then
    echo "  would drop: $AGENTS_DEST (skill bridge for codex/hermes)"
  else
    {
      printf '# Agent instructions (generated by shipyard install)\n\n'
      printf 'A non-Claude harness (codex or hermes) backs at least one role in\n'
      printf 'this project. The reusable skills for this project live under\n'
      printf '`.claude/skills/<name>/SKILL.md`. When a task prompt says "use the\n'
    ...
    echo "  wrote:     $AGENTS_DEST (skill bridge for codex/hermes)"
  fi
fi
