...
- **Created:** 2026-07-30
- **Owner:** wabbazzar
- **Status:** complete — native Mac, server Linux, CI, and Pages verified
- **Priority:** high
- **Type:** bugfix
...
## Server integration evidence (2026-07-30)

- Bundle SHA-256
  `a696f29cc38657beeafb6087dbb1f562ffe547773f5c6c6790215aa5ec53d315`
  verified; required base `a065ae1` and tip `d1365c6` matched the handoff.
- The five source patches and complete commit messages matched their
  server-integrated commits byte-for-byte. Raw author and committer fields were
  reconstructed as the canonical owner identity; the source work-domain
  metadata was not published.
- Linux Bats passed 565/565. Both `bash -n` and `/bin/bash -n` passed all 39
  tracked shell files; Python compiled all 3 tracked modules. Leak, deck
  freshness, deck completeness, lifecycle, identity, and `git diff --check`
  exited 0.
- Deck render exited 3 with the documented `playwright not available` skip.
  The direct installer dry-run exited 0 without writing.
- GitHub checks run `30569301636` passed all six jobs, including Bats, raw
  identity, shell, leak, deck, and ticket lifecycle. Pages run `30569300271`
  passed build, status reporting, and deployment.
...
- [x] A planted current-day event is counted inside the seven-day collector
...
- [x] The complete repository gate is green on the Mac, and Linux GitHub CI is
      green after server-side integration. Mac: 528/528; Linux: 565/565;
      CI run `30569301636` and Pages run `30569300271` passed.
...
| 3 — fixture utilities/final | Add named fixture mutation/mtime helpers, migrate every reproduced GNU-only fixture form, prove the static contract, run the full Mac gate, and graduate the ticket. | builder: subagent (1 agent) + lead integration | Mac `d1365c6`; server `4dd5c66` | RED: BSD `sed -i` aborted before the deck assertion; the first native full pass exposed Bash 3.2 runtime failures from nested `case` in command substitution and `${var^}`, GNU relative-date cooldown writes, lock-fixture mismatch, and `/var` ↔ `/private/var` manifest identity. GREEN: all 25 reproduced failures pass; native ARM64 `/bin/bash` Bats 528/528 (exit 0); native + modern syntax, Python compile, leak, deck freshness/completeness, and `git diff --check` pass. Render gate exits 3 only because Playwright is absent. Live LaunchAgents and event files were not mutated. Server integration: Linux Bats 565/565; syntax, Python compile, leak, deck, lifecycle, identity, diff, and installer dry-run gates passed; GitHub checks `30569301636` and Pages `30569300271` passed. |
